...
erDiagram
    ...
    FRIENDSHIP ||--|| DIRECT_CONVERSATION : enables
...
| `Event` | UUID, title, description, image, sport, level, languages, address, coordinates, dates, capacity, visibility | Created by a user and optionally associated with a group |
| `EventParticipant` | status, queue position, joined time | Joins users to events and implements attendance/waiting lists |
| `Group` | UUID, name, description, sport, levels, capacity, languages, location, cover image, active state | Owned by a user and contains members, messages, and events |
| `GroupMembership` | role, status, joined time | Joins users to groups with owner/admin/member roles |
...
| `DirectConversation` | UUID, friendship, timestamps | One conversation for an accepted friendship |
...
| `Notification` | recipient, actor, type, payload, target URL, read time | Records social, message, event, group, and membership notifications |
...
| `PublicAPIKey` | name, prefix, key hash, status, timestamps | Authenticates and rate-limits public API clients without storing raw keys |
